...
ユースケース（想定する運用像）
- 実行者はすでにCSVを持っている状態で、ターミナル/コマンドプロンプトで本スクリプトを実行します。
- 依存ライブラリは `requirements.txt` で一括インストールできます。
...
使い方（例）
  python analyze_sales.py \
      --input sample_sales_data.csv \　# 入力CSVのファイルパスです。
      --outdir output \　# 生成物（CSV/HTML）の出力ディレクトリです。
      --start 2022-01-01 --end 2023-12-31 \　# 期間フィルタの開始日です（省略可）です。
      --open-html　# 出力後にHTMLを既定ブラウザで開くかどうかのフラグです。

出力物（主なCSV）
- monthly_summary.csv, customer_group_summary.csv, store_summary.csv, # 月次、法人グループ、店舗の売上データ
  product_summary.csv, prefecture_summary.csv, reps_customer_summary.csv, # 製品、都道府県、担当者の売上データ
  reps_company_summary.csv, price_quantity_bins.csv, quality_issues.csv # 担当者、価格×数量、論理整合性チェック結果
